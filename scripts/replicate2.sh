...
# Sized for a 1×H100 node: lr_sweep then alpha_sweep.
#
# Usage:
#   scripts/replicate2.sh <mamba-env>
#     e.g. scripts/replicate2.sh ml13
#
# Knobs (env vars):
#   WORKERS_PER_GPU=8 scripts/replicate2.sh ml13     # back off if OOM
#   SESSION=repl2_v2 scripts/replicate2.sh ml13      # custom tmux session
#   GC_WALLOW_DB=/path/to/runs.db ...                # override DB location
#
...
# live on local disk — we default GC_WALLOW_DB to $HOME/grokking_capacity_runs.db.
#
